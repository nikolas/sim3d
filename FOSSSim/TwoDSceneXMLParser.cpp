...
void TwoDSceneXMLParser::loadSceneTag( rapidxml::xml_node<>* node, std::string& scenetag )
{
  ...
  //std::cout << "Calling loadSceneTag" << std::endl;
...
void TwoDSceneXMLParser::loadCameraLocation(
    rapidxml::xml_node<>* node, TwoDimensionalDisplayController& controller) {
  assert(node != NULL);
  for (rapidxml::xml_node<>* nd = node->first_node("camera"); nd;
       nd = nd->next_sibling("camera")) {
    float theta, phi, r;
    if (nd->first_attribute("theta")) {
      ...
      if (!stringutils::extractFromString(attribute, theta)) {
        std::cerr << "Could not parse theta value for camera." << std::endl;
      }
    } else {
      std::cerr << "\033[31;1mERROR IN XMLSCENEPARSER:\033[m Failed to parse theta for camera." << std::endl;
      exit(1);
    }

    if (nd->first_attribute("phi")) {
      std::string attribute(nd->first_attribute("phi")->value());
      if (!stringutils::extractFromString(attribute, phi)) {
        std::cerr << "Could not parse phi value for camera." << std::endl;
      }
    } else {
      std::cerr << "\033[31;1mERROR IN XMLSCENEPARSER:\033[m Failed to parse phi for camera." << std::endl;
      exit(1);
    }
    ...
    if (nd->first_attribute("r")) {
      ...
      if (!stringutils::extractFromString(attribute, r)) {
        std::cerr << "Could not parse r value for camera." << std::endl;
      }
    } else {
      std::cerr << "\033[31;1mERROR IN XMLSCENEPARSER:\033[m Failed to parse r for camera." << std::endl;
      exit(1);
    }

    controller.setCamera(M_PI * theta / 180, M_PI * phi / 180, r);
  }
}
